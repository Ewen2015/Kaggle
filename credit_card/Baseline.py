...
class Baseline(object):
    ...
    def LR(self, report=False):
        ...
        if report:
            ...
            rpt = Report(self.lr, self.train_prep, self.valid_prep, self.target, self.features, is_sklearn=True)
            rpt.ALL()
    ...
    def RF(self, report=False):
        ...
        if report:
            from Report import Report
            rpt = Report(self.rf, self.train_prep, self.valid_prep, self.target, self.features, is_sklearn=True)
            rpt.ALL()
    ...
    def GBDT(self, report=False):
        ...
        if report:
            from Report import Report
            rpt = Report(self.gbdt, self.train, self.valid, self.target, self.features, is_sklearn=True)
            rpt.ALL()
    ...
    def NN(self, report=False):
        ...
        self.nn = KerasClassifier(build_fn=baseline_model, epochs=50, batch_size=100, verbose=1)
        self.nn.fit(self.train[self.features], self.train[self.target])

        if report:
            from Report import Report
            rpt = Report(self.nn, self.train, self.valid, self.target, self.features, is_sklearn=True)
            rpt.ALL()
