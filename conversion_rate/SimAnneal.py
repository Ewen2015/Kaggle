...
class SimulatedAnneal(object):
    ...
    def __init__(self, estimator, param_grid, scoring='roc_auc',
                 ...
                 max_iter=300, max_runtime=300, cv=3, random_state=2018,
                 verbose=False, refit=True, max_score=np.inf, n_jobs=1):
    ...
    def fit(self, dtrain, dvalidation, target, features):
        ...
        # Computes the training X, y
        X_train, y_train = dtrain[features], dtrain[target]
        ...
        old_score, old_std = CVFolds(old_est, scorer=score_func, cv=cv).fit_score(dtrain, dvalidation, target, features)
            ...
            while iter_ < n_trans and new_score < self.__max_score:
                ...
                try:
                    ...
                    new_score, new_std = MultiProcCvFolds(new_est, score_func, cv, self.__n_jobs,
                                                          self.__verbose).fit_score(X_train, y_train )
        ...
        if self.__refit:
            ...
            self.__est.fit(X_train, y_train, eval_set=(dvalidation[features], dvalidation[target]), verbose=False)
...
class CVFolds(object):
    ...
    def fit_score(self, dtrain, dvalidation, target, features):
        cross_valid_1 = KFold(self.__cv).split(dtrain[features].as_matrix())
        cross_valid_2 = KFold(self.__cv).split(dvalidation[features].as_matrix())
        
        scorer = self.__scorer
        scores = []
        
        for train_index, _ in cross_valid_1:
            X_train = dtrain[features].loc[train_index,:]
            y_train = dtrain[target][train_index]

            for _, test_index in cross_valid_2:
                X_test = dvalidation[features].loc[test_index,:]
                y_test = dvalidation[target][test_index]
